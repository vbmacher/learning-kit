...
import student.constraints.binary.DoNotOverlap;
...
public class CSPMain {
    ...
    static boolean fillBlockOnGrid(String[][] grid, Selection sel, int startPos) {
        Block block = sel.block();

//        System.out.println("drawing dimension = " + sel.dimension);

        // verify if we can put selected block on the grid
        GridDimension dim = sel.toGridDimension(startPos);
        // System.out.println("grid - " + dim.row + "," + dim.col);
...
//                );
                ...
                boolean xValid = assignment.isBinaryConsistent(x, y);
                //assignment.isConsistent(x, gridCloneSecond)
...
//            }
    ...
    private static boolean ac3(String[][] grid, Assignment assignment, Domains domains) {
        ...
        int domainsCountBefore = domains.count();
        ...
        while (!queue.isEmpty()) {
            ...
            for (Selection y : assignment.relatedVariables(x, domains)) {
                if (removeValues(grid, x, y, domains)) {
                    if (domains.domain(y).isEmpty() && y.block().length > 0) {
                        int domainsCountAfter = domains.count();

                        if (domainsCountAfter < domainsCountBefore) {
                            System.out.println("(F) AC3 REDUCED TO: " + (domainsCountBefore - domainsCountAfter));
                        }
                        return false;
                    }
                    if (!queue.contains(y)) {
        ...
        if (domainsCountAfter < domainsCountBefore) {
            System.out.println("AC3 REDUCED TO: " + (domainsCountBefore - domainsCountAfter));
        }
        return true;
    }
...
//                if (length > 0) {
  ...
  //              }
                ...
                blocks[dimension][index][blockIndex] = new Block(length, color);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        Assignment assignment = new Assignment(blocks, Arrays.asList(
                ...
                new DoNotOverlap()
//                new OverlappingTransposedBlocksShareTheColor()
        ));
        ...
        // assign variables for zero-length blocks
