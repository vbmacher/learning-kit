...
public class CSPMain {
    ...
    private static boolean sequenceDoesNotOverlap(int[][] var, Block[][] domain, int index, int blockIndex) {
        for (int k = Math.max(blockIndex - 1, 0); k < Math.min(blockIndex + 1, domain[index].length - 1); k++) {
    ...
    private static boolean removeValues(final String[][] grid, Selection x, Selection y,
        ...
        for (int yValue : yStarts) {
            ...
            boolean yValueIsValid = sequenceDoesNotOverlap(varY, yDomain, y.index, y.blockIndex)
            ...
            for (int xValue : xBlock.starts) {
                ...
                boolean xValid = sequenceDoesNotOverlap(varX, xDomain, x.index, x.blockIndex)
    ...
    private static int forwardChecking(boolean checkRowVar, String[][] grid, int[][] varRow, int[][] varCol,
        ...
        for (Selection sel : findFreeVars(checkRowVar, var, domain).values()) {
            ...
            for (int value : block.starts) {
                ...
                boolean isValid = sequenceDoesNotOverlap(var, domain, sel.index, sel.blockIndex)
    ...
    private static boolean csp(String[][] grid, int[][] varRow, int[][] varCol, Block[][] domainRow, Block[][] domainCol) {
        ...
        class LeastConstrainingValue {
            ...
            private final Block[][] domainRow;
            private final Block[][] domainCol;

            private LeastConstrainingValue(int value, Block[][] domainRow, Block[][] domainCol) {
                this.value = value;
        ...
        for (int value : block.starts) {
            ...
            if (!leastConstValues.containsKey(count)) {
                leastConstValues.put(count, new ArrayList<>());
            }
            leastConstValues.get(count).add(new LeastConstrainingValue(value, domainRowBackup, domainColBackup));
        ...
        for (Map.Entry<Integer, List<LeastConstrainingValue>> lcValueList : leastConstValues.entrySet()) {
            for (LeastConstrainingValue lcValue : lcValueList.getValue()) {
                String[][] gridBackup = cloneGrid(grid);
                if (!assign(sel, var, grid, lcValue.value)) {
                    continue;
                }
                foundAtLeastOne = foundAtLeastOne | csp(grid, varRow, varCol, lcValue.domainRow, lcValue.domainCol);
