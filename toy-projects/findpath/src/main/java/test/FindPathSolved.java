...
public class FindPathSolved {
    private final static int OBSTACLE = -1;
    private final int[][] map; // N x N

    @SuppressWarnings("unchecked")
    private FindPathSolved(List<List<Integer>> map) {
    ...
    public static FindPathSolved fromMap(String map) {
        ...
        return new FindPathSolved(parsedMap);
    }
    ...
    public int minDistance(Point start, Point stop) {
        ...
        while (!toDiscover.isEmpty()) {
            ...
            Point left = left(current);
            if (canGo(left) && isWorthGoing(distances, left, current)) {
                distances[left.y][left.x] = nextDistance;
                toDiscover.add(left);
            }

            Point right = right(current);
            if (canGo(right) && isWorthGoing(distances, right, current)) {
                distances[right.y][right.x] = nextDistance;
                toDiscover.add(right);
            }

            Point up = up(current);
            if (canGo(up) && isWorthGoing(distances, up, current)) {
                distances[up.y][up.x] = nextDistance;
                toDiscover.add(up);
            }

            Point down = down(current);
            if (canGo(down) && isWorthGoing(distances, down, current)) {
                distances[down.y][down.x] = nextDistance;
                toDiscover.add(down);
            }
        }
