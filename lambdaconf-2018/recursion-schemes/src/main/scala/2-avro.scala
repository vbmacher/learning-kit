...
trait UsingARegistry {
  ...
  // Monadic algebra - Instead of using hylo (explicit refolding), we will benefit from flatMap!
  // The benefit is as follows:
  //   - hylo unfolds original schema pattern-functor to labelled (one traversal), then folds it to Schema (second traversal)
  //   - monadic algebra/coalgebra uses monads "flatmaps" instead of explicit fold, which implicitly remembers previous results
  //     so just one traversal is needed overall
  ...
  implicit def schemaFTraverse: Traverse[SchemaF] = new Traverse[SchemaF] {
    override def traverseImpl[G[_], A, B](fa: SchemaF[A])(f: A => G[B])(implicit G: Applicative[G]): G[SchemaF[B]] =
      fa match {
        case StructF(fields) =>
          //   - traverses the list of fields using the function `f` to get List[G[B]] similarly than a map,
          //     where G[_] is Applicative and it is used somehow to combine G[B]'s in a way of
          //     "flipping" List[G[_]] to G[List[_]] (building with G.`pure` and then applying G.ap)
          //
          //   - just to recall: Applicative.ap:    ap:  (F[A =>B], F[A]): F[B]

          val (ks, vs) = fields.unzip
          //val traversedList = G.traverse(vs.toList)(f)

          //also works:  vs.toList.traverse(f).map(xs => StructF(ListMap(ks.toList zip xs: _*)))
          //traversedList.map(xs => StructF(ListMap(ks.toList zip xs: _*)))
          vs.toList.traverse(f).map(xs => StructF(ListMap(ks.toList zip xs: _*)))
...
trait AvroCoalgebra {
  ...
  /**
    ...
    */
  def avroToSchemaF: CoalgebraM[Option, SchemaF, Schema] = new CoalgebraM[Option, SchemaF, Schema] {
