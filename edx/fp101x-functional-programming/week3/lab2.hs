...
toDigits 0 = [0]
toDigits n = reverse (digitsRev n)
...
doubleSecond [] = []
doubleSecond (x:xs) = x : (justDouble xs)
  where justDouble [] = []
        justDouble (x:xs) = (x * 2) : doubleSecond xs
...
testToDigits n = (eval (toDigits n) == n) && (all (\d -> d >= 0 && d < 10) (toDigits n))
  && (length (show n) == length (toDigits n))


evalRev xs = foldr (\x y -> x + (10 * y)) 0 xs

testToDigitsRev n = (n == evalRev (toDigitsRev n)) && (all (\d -> d >=0 && d < 10) (toDigitsRev n))
