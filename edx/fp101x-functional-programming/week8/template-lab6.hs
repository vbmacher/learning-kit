...
leaves :: Rose a -> Int
leaves (x :> []) = 1
leaves (x :> xs) = sum $ map countleaves xs
  where countleaves (_ :> []) = 1
        countleaves (_ :> xs) = sum $ map countleaves xs
...
instance Foldable Rose where
  fold (x :> []) = x
  fold (x :> xs) = x `mappend` (msum $ map fold xs)
    where msum (x : []) = x
          msum (x : xs) = mappend x $ msum xs
...
fsum a = unSum $ foldMap (\b -> Sum b) a
fproduct a = unProduct $ foldMap (\b -> Product b) a
